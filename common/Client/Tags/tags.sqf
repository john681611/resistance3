   ...
   // PLAYER NAME CHECK AND DISPLAY
        ...
        if (_target isKindOf "Man" && player == vehicle player) then{
				...
				_name = name _target;
						...
						_weaponsplayername = getText (configFile >> "CfgVehicles" >> (typeOf _target) >> "displayName");

	// PRINT THE RANK/NAME/WEAPON ONSCREEN

						_nameString = format ["<t size='0.5' color='#f0e68c'>%1</t><br/><t size='0.5' color='#f0e68c'>%2</t><br/>",_name, _weaponsplayername];
	...
	// SETTING UP THE NAME AND IMAGE FORMATION
								_driver = format ["%1 %2 <img size='0.35' color='#7FFF70' image='Client\Tags\images\crew_driver.paa'/><br/>",_blank, _driver];
								_gunner = format ["%1 %2 <img size='0.35' color='#7FFF70' image='Client\Tags\images\crew_gunner.paa'/><br/>",_blank,_gunner];
								_commander = format ["%1 %2 <img size='0.35' color='#7FFF70' image='Client\Tags\images\crew_commander.paa'/><br/>",_blank,_commander];
								_cargo = format ["%1 / %2 <img size='0.35' color='#7FFF70' image='Client\Tags\images\crew_cargo.paa'/><br/>",_freePassengerSpaces, _passengerSpaces];
	...
	// PRINTING OUT ON SCREEN. TO MOVE POSITION, CHANGE THE X,Y CORODINATES AFTER _printname WITHIN THE DYNAMICTEXT
        ...
        #ifdef _debug
        diag_log  format["time: %1, frames: %2",_initTime - diag_tickTime,_frameNo - diag_frameNo];
        #endif
        sleep _refresh;
};
