...
if (!(getmarkercolor _mkr == "colorblack")) then {
	...
	for "_counter" from 1 to _aGrps do {
		...
		if (_aMin > 0) then {
			_pos = [_mkr, 2] call EOS_fnc_findSafePos;
			_aGroup = [_pos, _aSize, _faction, _side, false] call EOS_fnc_spawngroup;
			[_aGroup, _mPos, _mkrX, 3, 0.2, 0.4] call CBA_fnc_taskDefend;
	...
	for "_counter" from 1 to _bGrps do {
		...
		if (_bMin > 0) then {
			...
			[_bGroup, _mPos, _mkrX, 1, 0.9, 0.1] call CBA_fnc_taskDefend;
	...
	for "_counter" from 1 to _cGrps do {
		...
		_newpos = [_mkr, 6] call EOS_fnc_findSafePos;
		...
		[_cGroup select 2, _mPos, _mkrX, 0, 1, 0] call CBA_fnc_taskDefend;
	...
	for "_counter" from 1 to _dGrps do {
		...
		[_dGroup select 2, _mPos, _mkrX, 0, 1, 0] call CBA_fnc_taskDefend;
	...
	for "_counter" from 1 to _fGrps do {
		...
		if ((_fSize select 0) > 0) then {
			...
			[_fGroup select 2, _mPos, _mkrX, 0, 1, 0] call CBA_fnc_taskDefend;
		};
	...
	for "_counter" from 1 to _gGrps do {
		...
		_newpos = [_mkr, 3] call EOS_fnc_findSafePos;
		...
		_i = 0;
		while{(count nearestObjects[(_gGroup select 0), ["Man", "ReammoBox_F"], 1]) > 1 AND _i <= 5
	} do {
		0 = [_mPos, [(_gGroup select 0)], _mkrX, 0, [0, 20], true, true] call shk_fnc_fillhouse;
		_i = _i + 1;
	};
...
while { sleep 5; _eosAct } do {
	...
	if (_delay <= 0) then	{
		...
		if (!isnil "_cGrp") then
		{
			...
			} foreach _crew) then { _cGrps = _cGrps - 1; };
	...
	if (!isnil "_dGrp") then {
		{
			_vehicle = _x select 0; _crew = _x select 1; _grp = _x select 2;
			if (!alive _vehicle || {!alive _x
		} foreach _crew) then { _dGrps = _dGrps - 1; };
		{ deleteVehicle _x } forEach(_crew);
		if (!(vehicle player == _vehicle) && _vehicle in list _eosActivated) then { { deleteVehicle _x } forEach[_vehicle]; };
		{ deleteVehicle _x } foreach units _grp; deleteGroup _grp;
	} foreach _dGrp;
...
if (!isnil "_eGrp") then {
	{
		_vehicle = _x select 0; _crew = _x select 1; _grp = _x select 2;
		if (!alive _vehicle || {!alive _x
	} foreach _crew) then { _eGrps = _eGrps - 1; };
	{ deleteVehicle _x } forEach(_crew);
	if ((!(vehicle player == _vehicle)) && _vehicle in list _eosActivated) then { { deleteVehicle _x } forEach[_vehicle]; };
	{ deleteVehicle _x } foreach units _grp; deleteGroup _grp;
} foreach _eGrp;
...
if (!isnil "_fGrp") then {
	{
		...
		if (!alive _vehicle || {!alive _x
	} foreach _crew) then { _fGrps = _fGrps - 1; };
	{ deleteVehicle _x } forEach(_crew);
	if (!(vehicle player == _vehicle) && _vehicle in list _eosActivated) then { { deleteVehicle _x } forEach[_vehicle]; };
	{ deleteVehicle _x } foreach units _grp; deleteGroup _grp;
