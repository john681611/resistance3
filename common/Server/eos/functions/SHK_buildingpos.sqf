...
if isServer then {
	...
	if (typeName _opos == typeName objNull) then {
		_opos = getPos _opos
	};
	_men = _this select 1;
	_rad = if (count _this > 2) then {
		_this select 2
	} else {
		20
	};
...
_disableMove = if (count _this > 5) then {
	_this select 5
} else {
	false
};
...
_taken = [];
{
	...
	while { count _taken < count _bpos } do {
		switch _prio do {
			case 0: {
				_ind = floor(random count _bpos)
			}; // random
			 case 1: {
				_ind = count _taken
			}; // ground floor first
			 case 2: {
				_ind = count _bpos - count _taken - 1
			}; // roof first
		};
...
if (count _this > 7) then {
	...
	if isServer then {
		...
		if !isDedicated then {
			[true, _hidden] call SHK_BuildingPos_fnc
		};
		...
		publicvariable "SHK_BuildingPos_EH";
		if !isDedicated then {
			[false, _hidden] call SHK_BuildingPos_fnc
		};
	};
};