...
getbox = {
  ...
  _data = (((profilenamespace getvariable "Resist_Box") select 1)select 2);
...
setbox = {
  _box = (((profilenamespace getvariable "Resist_Box") select 1)select 1);
  ...
  profilenamespace setvariable ["Resist_Box",[KeyNumber,[true,"none",_content]]];
...
tryGetBox = {
  if((((profilenamespace getvariable "Resist_Box") select 1)select 0))then{
    [] spawn getbox;
  }else{
