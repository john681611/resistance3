...
switch (_faction)
	...
	case 8:
	{
		_InfPool = RHS_USARMY_W_InfPool + [];
		_ArmPool = RHS_USARMY_W_ArmPool + [];
		_MotPool = RHS_USARMY_W_MotPool + [];
		_ACHPool =RHS_USARMY_W_ACHPool + [];
		_CHPool = RHS_USARMY_W_CHPool + [];
		_uavPool = RHS_USARMY_W_uavPool + [];
		_stPool = RHS_USARMY_W_stPool + [];
		_shipPool = RHS_USARMY_W_shipPool + [];
		_diverPool = RHS_USARMY_W_diverPool + [];
