...
#define DEFAULT_FREPAIR_VER "1m"

#define DEFAULT_FULLREPAIR_LENGTH 180
#define DEFAULT_FIELDREPAIR_EACH_PART_TIME 40
...
zlt_frpr_getPartsRepairTime = {
	...
	_rprTime = 0;
	{
		...
		if (not isNil {_cdmg} ) then {
			...
			if (_cdmg > 0.64) exitWith {_rprTime = _rprTime + DEFAULT_FIELDREPAIR_EACH_PART_TIME;};
		};
	}  forEach zlt_repair_hps;
	_rprTime;
};
...
zlt_fnc_heavyRepair = {
	...
	_caller = player; //): Object - the unit that activated the action 
	...
	if (not alive player or vehicle _caller == _caller or speed _veh > 3 or driver _veh != _caller or _veh distance _truck > 15 ) exitWith {STR_REPAIR_CONDITIONS call zlt_fnc_notify;};
	...
	_vehname = getText ( configFile >> "CfgVehicles" >> typeOf(_veh) >> "displayName");
	_length = _maxlength;
	while { alive player and alive _truck and alive _veh and driver _veh == _caller and speed _veh <= 3 and not _repairFinished and zlt_mutexAction and _veh distance _truck <= 15 } do {			
		(format[STR_REPAIR_MSG_STRING, _length, _vehname] ) call zlt_fnc_notify;
		if (_length <= 0) then {_repairFinished = true;};
		_length = _length - 1;
		sleep 1;
...
zlt_fnc_heavyRepairCOnd = {
	...
	_veh = vehicle player;
	_res = _veh distance _truck <= 15 and {player == (driver _veh)} and {(_veh isKindOf "LandVehicle" or _veh isKindOf "Ship" or _veh isKindOf "Air")} and {alive _truck} and {alive player} and {alive _veh} and {not zlt_mutexAction} and {speed _veh <= 3} and {(damage _veh != 0)};
	_res;
...
if (isNil "zlt_cancelActionId") then {
	...
	player addAction["<t color='#ff0000'>"+STR_FIELD_REPAIR+"</t>", zlt_prc_repairvehicle, [], -1, false, false, '','[] call zlt_fnc_repair_cond'];
	player addAction["<t color='#ff0000'>"+STR_SERIOUS_REPAIR+ "</t>", zlt_fnc_heavyRepair, [], -1, false, false, '','[] call zlt_fnc_heavyRepairCOnd'];
	player addAction["<t color='#FF9900'>"+STR_PUSH_APC+"</t>",zlt_pushapc,[],5,false,false,"","canMove (vehicle player)  and player == driver (vehicle player) and surfaceIsWater getpos (vehicle player)  and abs(speed (vehicle player) ) < 3 and not zlt_mutexAction"];   
...
player addEventHandler ["Respawn", {
	...
	player addAction["<t color='#ff0000'>"+STR_FIELD_REPAIR+"</t>", zlt_prc_repairvehicle, [], -1, false, false, '','[] call zlt_fnc_repair_cond'];
	player addAction["<t color='#ff0000'>"+STR_SERIOUS_REPAIR+ "</t>", zlt_fnc_heavyRepair, [], -1, false, false, '','[] call zlt_fnc_heavyRepairCOnd'];
	player addAction["<t color='#FF9900'>"+STR_PUSH_APC+"</t>",zlt_pushapc,[],5,false,false,"","canMove (vehicle player) and ((vehicle player) isKindOf 'Wheeled_APC_F') and player == driver (vehicle player) and surfaceIsWater getpos (vehicle player)  and abs(speed (vehicle player) ) < 3 and not zlt_mutexAction"];   
}];
