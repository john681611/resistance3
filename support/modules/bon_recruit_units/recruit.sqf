...
if(_queuecount > 0) then {
	...
	hint parseText format["<t size='1.0' color='#ef2525'>%1</t> added to queue.",_typename];
};
...
hint parseText format["Processing your <t size='1.0' color='#ffd800'>%1</t>.",_typename];
...
hint parseText format["Your <t size='1.0' color='#008aff'>%1</t> %2 has arrived.",_typename,name _unit];
bon_recruit_queue = bon_recruit_queue - [_queuepos];
